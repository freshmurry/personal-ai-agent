...
app.post('/api/history', async (c) => {
  ...
  for (const msg of messages.slice(-100)) {
    ...
    ).bind(msg.role, msg.content, msg.ts || Date.now()).run()
  }
  return c.json({ ok: true })
})
...
// ── Agent WebSocket + HTTP (routeAgentRequest handles /agents/* routes) ───────
// This is the critical piece that connects the Agents SDK to the Hono router.
// WebSocket upgrades to /agents/super-agent/default are handled here.
...
export default {
  async fetch(request: Request, env: Bindings, ctx: ExecutionContext): Promise<Response> {
    ...
    // Route all /agents/* traffic through the Cloudflare Agents SDK
    if (url.pathname.startsWith('/agents/')) {
      return (await routeAgentRequest(request, env)) ?? new Response('Not found', { status: 404 })
    }

    // All other traffic goes through Hono
